...
It is sometimes suggested that one take the log-transform of outcomes which display evidence of heteroskedasticity. I wrote these simulations some time in 2019 for a lab presentation where I was trying to illustrate the consequences of doing so when the true outcome is linear. About a year later, I saw a tweet by Rachel Meager (@economeager) which summed up what I wantedto say much better than I ever could have.

"Also remember if you are someonewho cares about bias in your estimator that you can be unbiased for the logmean or for the raw mean but not both" 

See the tweet [here](https://twitter.com/economeager/status/1319250445123096576?s=20&t=fNsM24903WtwwsmkzId8vA).

So here I start with unbiased (but inefficient) estimation on the raw mean and we see the bias when we incorrectly use the log transform. Of course, we could do this in the other direction as well.
...
Here we have classical proportional heteroskedasticity. Now suppose we used a log transformation
